...
function CategoryList() {
    ...
    const fetchData = async () => {
        try {
            const categoryResponse = await fetch('http://localhost:8080/category/allCategory');
            const subcategoryResponse = await fetch('http://localhost:8080/subcategory/allSubCategory');
    ...
    const handleUpdate = async (updatedData, param) => {
        try {
            ...
            if(param === 'subCategoryData') {
                apirUrl = 'http://localhost:8080/subcategory/update';
            } else if(param === 'categoryData'){
                apirUrl = 'http://localhost:8080/category/update';
            }
    ...
    const handleDelete = async (subcategory) => {
        try {
            ...
            const response = await fetch(`http://localhost:8080/subcategory/delete/${subcategory.id}`, { // Use subcategory.id
            method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json'
                }
            ...
            if (response.ok) {
                ...
                console.error('Failed to delete subcategory already delete:', response.statusText);
            }
        } catch (error) {
    ...
    const confirmDelete = async () => {
        try {
            const response = await fetch(`http://localhost:8080/category/delete/${selectedItem.id}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json'
                }
            ...
            if (response.ok) {
                ...
                setCategoryData(prevCategoryData => prevCategoryData.filter(category => category.id !== selectedItem.id));
            ...
            {categoryData.map(category => (
                <div className="category" key={category.id}>
                    <div className="category-header">
                        ...
                        <FaPencilAlt className="edit-icon" onClick={() => handleEdit(category)} />
                    </div>
                    <div className="subcategory-list">
                        {category.subcategories.map(subcategory => (
                            <div className="subcategory" key={subcategory.id}>
                                ...
                                <FaPencilAlt className="edit-icon" onClick={() => handleEdit(subcategory)} />
                                <div className="action-icons">
                                    <FaTrash className="delete-icon" onClick={() => handleDelete(subcategory)} />
                                </div>
                            </div>
